...
def _describe_feature_impact(feature: str, value: float, contribution: float, verdict: str) -> Optional[str]:
    ...
    if feature == 'dissolved_oxygen':
        if contribution > 0:
            return f"✓ Dissolved oxygen ({value:.1f} mg/L) is at a healthy level"
        else:
            return f"⚠ Dissolved oxygen ({value:.1f} mg/L) is below optimal levels"

    elif feature == 'ph':
        if 6.5 <= value <= 8.5:
            return f"✓ pH ({value:.1f}) is within ideal range (6.5-8.5)"
        elif contribution < 0:
            if value < 6.5:
                return f"⚠ pH ({value:.1f}) is too acidic (ideal: 6.5-8.5)"
            else:
                return f"⚠ pH ({value:.1f}) is too alkaline (ideal: 6.5-8.5)"

    elif feature == 'temperature':
        if contribution > 0 and 10 <= value <= 20:
            return f"✓ Temperature ({value:.1f}°C) is in optimal range"
        elif contribution < 0 and value > 25:
            return f"⚠ Temperature ({value:.1f}°C) is elevated (ideal: <25°C)"

    elif feature == 'nitrate':
        if contribution > 0 and value < 5:
            return f"✓ Nitrate ({value:.1f} mg/L) is low (EPA MCL: 10 mg/L)"
        elif contribution < 0 and value >= 10:
            return f"⚠ Nitrate ({value:.1f} mg/L) exceeds EPA MCL (10 mg/L)"
        elif contribution < 0:
            return f"⚠ Nitrate ({value:.1f} mg/L) is elevated"

    elif feature == 'turbidity':
        if contribution > 0 and value < 1:
            return f"✓ Turbidity ({value:.1f} NTU) is very low (EPA standard: <1 NTU)"
        elif contribution < 0 and value > 5:
            return f"⚠ Turbidity ({value:.1f} NTU) is high (WHO ideal: <5 NTU)"

    elif feature == 'conductance':
        if contribution > 0 and 150 <= value <= 500:
            return f"✓ Conductivity ({value:.0f} µS/cm) indicates moderate mineral content"
        elif contribution < 0 and value > 800:
            return f"⚠ Conductivity ({value:.0f} µS/cm) is very high (high mineral/salt content)"

    # Derived features
    elif feature == 'ph_deviation_from_7':
        if contribution < 0 and value > 2:
            return f"⚠ pH deviation from neutral (±{value:.1f}) is significant"

    elif feature == 'do_temp_ratio':
        if contribution > 0:
            return f"✓ Dissolved oxygen relative to temperature is favorable"
        elif contribution < 0:
            return f"⚠ Dissolved oxygen is low relative to water temperature"

    elif feature == 'pollution_stress':
        if contribution < 0:
            return f"⚠ Combined nitrate and oxygen stress indicates contamination"

    elif feature == 'n_params_available':
        if contribution > 0 and value >= 5:
            return f"✓ Comprehensive parameter coverage ({int(value)} of 6 parameters available)"
...
def _assess_water_parameters(params: Dict[str, float]) -> Dict[str, Dict[str, str]]:
    ...
    if not np.isnan(temp):
        if 10 <= temp <= 20:
            ...
            assessment['Temperature'] = {'status': 'concerning', 'reason': f'{temp:.1f}°C exceeds WHO preference (<25°C)'}
        else:
    ...
    if not np.isnan(conductance):
        if 150 <= conductance <= 500:
            ...
            assessment['Conductivity'] = {'status': 'concerning', 'reason': f'{conductance:.0f} µS/cm indicates high mineral/salt content'}
        else:
